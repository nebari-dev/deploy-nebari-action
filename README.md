...
- Exposes the platform's entry points as outputs via [`nic outputs`](https://github.com/nebari-dev/nebari-infrastructure-core/blob/main/docs/reference/cli/nic_outputs.md): the Keycloak and ArgoCD admin credentials (masked in logs), the gateway address, and the domain and Keycloak issuer URL. The command resolves all fields or none, so when any field cannot be resolved every platform output is an empty string and a warning names each unresolved field and why. Extraction never fails an otherwise successful deploy.
